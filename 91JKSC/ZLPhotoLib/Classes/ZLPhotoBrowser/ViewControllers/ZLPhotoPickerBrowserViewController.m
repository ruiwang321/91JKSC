...
@interface ZLPhotoPickerBrowserViewController () <UIScrollViewDelegate,ZLPhotoPickerPhotoScrollViewDelegate,UICollectionViewDataSource,UICollectionViewDelegate,RatingBarDelegate>
...
- (UICollectionView *)collectionView{
    if (!_collectionView) {
        ...
        _backview = [[UIView alloc]initWithFrame:CGRectMake(0, SYS_HEIGHT-100, SYS_WIDTH, 100)];
        _backview.backgroundColor = [UIColor blackColor];
        _backview.alpha = 0.8;
        [self.view addSubview:_backview];
        
        RatingBar *ratingBar1 = [[RatingBar alloc] init];
        ratingBar1.frame = CGRectMake(10, 10, 150, 25);
        ratingBar1.tag = 10;
        [_backview addSubview:ratingBar1];
        ratingBar1.isIndicator = NO;//指示器，就不能滑动了，只显示评分结果
        [ratingBar1 setImageDeselected:@"star-defalt" halfSelected:nil fullSelected:@"star" andDelegate:nil];
        ratingBar1.isIndicator = YES;//指示器，就不能滑动了，只显示评分结果
        [ratingBar1 displayRating:self.evaluateModel.geval_scores.floatValue  andView:_backview];
        
        UILabel *titlesLabel = [[UILabel alloc] initWithFrame:CGRectMake(10, 45, SYS_WIDTH-20, 50)];
        titlesLabel.font = SYS_FONT(16);
        titlesLabel.backgroundColor = [UIColor clearColor];
        titlesLabel.textColor = [UIColor whiteColor];
        titlesLabel.text = self.evaluateModel.comment_message;
        [_backview addSubview:titlesLabel];
        if (self.evaluateModel.geval_scores.length == 0||self.evaluateModel.comment_message.length == 0) {
...
//        [self.view bringSubviewToFront:_deleleBtn = deleleBtn];
...
- (void)viewWillAppear:(BOOL)animated{
    ...
    [leftBtn setImage:[UIImage imageNamed:@"img_arrow"] forState:UIControlStateNormal];
//    [leftBtn setTitleColor:[UIColor blackColor] forState:UIControlStateNormal];
//    [leftBtn setTitle:@"返回" forState:UIControlStateNormal];
    [leftBtn addTarget:self action:@selector(back) forControlEvents:UIControlEventTouchUpInside];
    leftBtn.titleLabel.font = SYS_FONT(16);
    self.navigationItem.leftBarButtonItem = [[UIBarButtonItem alloc]initWithCustomView:leftBtn];
    
    if (self.editing) {
        UIButton *rightBtn = [UIButton buttonWithType:UIButtonTypeCustom];
        rightBtn.frame = CGRectMake(0, 0, 44, 44);
        [rightBtn setImage:[UIImage imageNamed:@"nav_delete_btn"] forState:UIControlStateNormal];
...
- (void)reloadData{
    ...
    if ([self.delegate respondsToSelector:@selector(photoBrowserShowToolBarViewWithphotoBrowser:)]) {
        ...
        toolBarView.frame = CGRectMake(x, self.view.zl_height - 44, width, 64);
