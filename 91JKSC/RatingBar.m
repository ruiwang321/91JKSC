...
-(void)setImageDeselected:(NSString *)deselectedName halfSelected:(NSString *)halfSelectedName fullSelected:(NSString *)fullSelectedName andDelegate:(id<RatingBarDelegate>)delegate{
    ...
    [_s2 setFrame:CGRectMake(width+5,     0, width, height)];
    [_s3 setFrame:CGRectMake(2 * (width+5), 0, width, height)];
    [_s4 setFrame:CGRectMake(3 * (width+5), 0, width, height)];
    [_s5 setFrame:CGRectMake(4 * (width+5), 0, width, height)];
 ...
 *  @param rating 评分值
 */
-(void)displayRating:(float)rating andView:baseView{
    ...
    starRating = rating;
    lastRating = rating;
    [_delegate ratingChanged:rating andView:baseView];
}
...
-(void)touchesEnded:(NSSet *)touches withEvent:(UIEvent *)event{
    ...
    if (newRating != lastRating){
        [self displayRating:newRating andView:self];
    }
