...
@interface EditAddressViewController () <UITextFieldDelegate, UIPickerViewDataSource, UIPickerViewDelegate> {
...
- (void)createSubView {
    ...
    for (int i = 0; i < 4; i++) {
        ...
        textFiexd.text = @[_addressModel.true_name, _addressModel.mob_phone, @"", _addressModel.address][i];
    ...
    [_selAreaBtn setTitle:_addressModel.area_info forState:UIControlStateNormal];
    ...
    if (nameTF.text.length <=0 || phoneTF.text.length <=0 || areaTF.text.length <=0) {
       [MBProgressHUD showError:@"请输入完整信息"];
        ...
        if ([phoneTF.text characterAtIndex:0] == '1' && phoneTF.text.length == 11) {
            ...
            [LoadDate httpPost:[NSString stringWithFormat:@"%@%@",API_ADDRESS,@"edit_address.html"] param:params finish:^(NSData *data, NSDictionary *obj, NSError *error) {
                if ([obj[@"code"] isEqualToNumber:@200]) {
                    [MBProgressHUD showSuccess: @"编辑地址成功"];
